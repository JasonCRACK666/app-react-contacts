...
import { updateContact, getContact } from "../services/contactsServices";
...
const EditContact = () => {
  ...
  const loadingContact = async (id, token) => {
  ...
  const handleCreateContact = async (e) => {
    ...
    setIsLoading(true);
    const result = await updateContact(contact, id, token);
    setIsLoading(false);
      ...
      <div className="flex flex-col items-center w-full">
        ...
        <form className="p-6 w-96" onSubmit={handleCreateContact}>
          ...
          <div className="mb-4">
            ...
            <input
              className={`w-full px-2 py-1 text-white bg-gray-900 border-b-2 ${
                !contact.adress ? "opacity-10" : "opacity-100"
              } transition-opacity focus:opacity-100 border-green-500 outline-none block`}
              type="text"
              name="adress"
              onChange={handleChange}
              value={contact.adress ? contact.adress : ""}
              id="adress"
